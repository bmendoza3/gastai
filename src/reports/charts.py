...
from src.db.storage import get_spend_by_category
...
def spend_pie_chart(user_phone: Optional[str] = None, days_back: int = 7) -> BytesIO:
    """Gráfico de torta: gasto por categoría. Retorna PNG en BytesIO."""
    df = get_spend_by_category(user_phone=user_phone, days_back=days_back)

    if df.empty:
        return _empty_chart(f"Sin gastos en los últimos {days_back} días")
    ...
    period = f"últimos {days_back} días" if days_back != 30 else "último mes"
    ax.set_title(
...
def spend_bar_chart(user_phone: Optional[str] = None, days_back: int = 7) -> BytesIO:
